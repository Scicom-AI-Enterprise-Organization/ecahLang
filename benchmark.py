...
async def stress_test(index, url, model, start_token, start_token_length, max_tokens, timeout):
    json_data = {
        'model': model,
        'prompt': 'Hello! '*200,
...
async def run_stress_test(url, model, start_token, start_token_length, concurrency, max_tokens, timeout):
    tasks = [
        stress_test(i, url, model, start_token, start_token_length, max_tokens, timeout)
...
def main(url, model, start_token, start_token_length, save, max_tokens, timeout, concurrency_list):
    ...
    async def run_all():
        for concurrency in concurrency_values:
            ...
            results = await run_stress_test(url, model, start_token, start_token_length, concurrency, max_tokens, timeout)
