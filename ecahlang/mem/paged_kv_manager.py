...
class PagedKVCacheManager:
    """
    Paged KV cache manager with free-list block allocation.

    Allocates a flat paged KV cache on GPU and manages page allocation/freeing
    using a simple free-list. Auto-sizes from available GPU memory via pynvml.

    KV cache shape: (num_layers, max_blocks, 2, block_size, num_kv_heads, head_dim)

    Inspired by:
    - vLLM v1/core/kv_cache_manager.py + block_pool.py
    - SGLang mem_cache/memory_pool.py + allocator.py
    - simple-continuous-batching-flashinfer manager.py
    """
    ...
    def __init__(
        ...
        # Per-sequence repetition penalty mask: [max_batch_size, vocab_size]
        self.mask_penalty = torch.ones(self.seq_lens, vocab_size, dtype=dtype).cuda()

        # Auto-size KV cache from free GPU memory
        ...
        per_token_bytes = num_kv_heads * head_dim * self.dtype_size * 2  # K + V
        per_block_per_layer = block_size * per_token_bytes
        self.max_blocks = usable_mem // (num_layers * per_block_per_layer)

        # Allocate the flat paged KV cache
        # Shape: [num_layers, max_blocks, 2(K+V), block_size, num_kv_heads, head_dim]
    ...
    def allocate(self, batch_id, total_tokens):
        ...
        if len(self.free_blocks) < num_pages:
            raise RuntimeError(
                f"Not enough KV cache blocks: need {num_pages}, have {len(self.free_blocks)}"
            )
    ...
    def free(self, batch_id):
        """Return pages for a completed sequence back to the free list."""
        blocks = self.batch_to_blocks.pop(batch_id, [])
        self.free_blocks.extend(blocks)
        self.batch_to_page_lengths.pop(batch_id, None)
        self.batch_to_total_tokens.pop(batch_id, None)
    ...
    def get_append_metadata(self, batch_ids):
        """
        Returns (kv_indices, kv_indptr, kv_last_page_len) for FlashInfer.

        These tensors describe the page layout so FlashInfer knows where
        each sequence's K/V data lives in the flat paged cache.
        """
        kv_indices = []
        kv_indptr = [0]
        kv_last_page_len = []
    ...
    def append_paged_kv_cache(self, batch_ids, key, value, append_indptr, layer_idx):
        """Write K/V tensors into the paged cache for a given layer."""
    ...
    def append_tokens(self, batch_id, num_new_tokens):
        """Extend a sequence's page allocation by N new tokens."""
    ...
    def get_stats(self):
        """Return KV cache utilization statistics."""
